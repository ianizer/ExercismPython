...
NORMAL_TICKET_TYPE = 0
EXPRESS_TICKET_TYPE = 1
...
def add_me_to_the_queue(
    ...
    if ticket_type == NORMAL_TICKET_TYPE:
        normal_queue.append(person_name)
        return normal_queue

    if ticket_type == EXPRESS_TICKET_TYPE:
        express_queue.append(person_name)
        return express_queue

    # To allow the program to add more values for ticket types in future updates,
    # I choose to check each type separately.
    # If the given ticket_type value matches no known ticket value, we have an error.
    raise Exception(f"Invalid ticket type: {ticket_type} is an invalid ticket type.")
